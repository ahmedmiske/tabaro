// server/middlewares/otpMiddleware.js
const twilio = require("twilio");
const User = require("../models/user");
const asyncHandler = require("../utils/asyncHandler");
const { checkPhoneNumber } = require("../utils/otpUtils");

const useTwilio = process.env.USE_TWILIO === "true";
const accountSid = process.env.TWILIO_ACCOUNT_SID;
const authToken = process.env.TWILIO_AUTH_TOKEN;
const serviceId = process.env.TWILIO_SERVICE_ID;

if (useTwilio) {
  if (!accountSid) throw new Error("no accountSid was provided!");
  if (!authToken) throw new Error("no authToken was provided!");
  if (!serviceId) throw new Error("no serviceId was provided!");
}
const client = useTwilio ? twilio(accountSid, authToken) : null;

const sendOTPMiddleware = asyncHandler(async (req, res, next) => {
  const { phoneNumber } = req.body;
  try {
    checkPhoneNumber(phoneNumber);
  } catch (error) {
    res.status(400);
    return next(error);
  }

  const isReset = "reset" in req.query;
  const user = await User.findOne({ phoneNumber });

  if (user && !isReset) {
    res.status(400);
    throw new Error("Phone number already exists!");
  } else if (!user && isReset) {
    res.status(400);
    throw new Error("Phone number does not exist!");
  }

  if (!useTwilio) {
    // تطوير: تخطّي الإرسال
    return next();
  }

  try {
    const verification = await client.verify.v2
      .services(serviceId)
      .verifications.create({ to: "+222" + phoneNumber, channel: "sms" });

    if (verification.status === "pending") {
      req.verification = verification;
      return next();
    }
    if (verification.status === "max_attempts_reached") {
      return next(new Error("Max attempts reached!"));
    }
    console.log("SMS send-otp unknown error!", verification);
    return next(new Error("SMS unknown error!"));
  } catch (error) {
    console.error(error);
    throw new Error("internal server error");
  }
});

const verifyOTPMiddleware = asyncHandler(async (req, res, next) => {
  const { phoneNumber, otp } = req.body;
  try {
    checkPhoneNumber(phoneNumber);
  } catch (error) {
    res.status(400);
    return next(error);
  }

  if (!useTwilio) {
    // تطوير: التحقق سيتم في الكنترولر
    return next();
  }

  try {
    const verification = await client.verify.v2
      .services(serviceId)
      .verificationChecks.create({ to: "+222" + phoneNumber, code: otp });

    if (verification.status === "approved") {
      req.verification = verification;
      return next();
    }

    if (["max_attempts_reached", "deleted", "failed", "expired"].includes(verification.status)) {
      return next(new Error(`${verification.status.charAt(0).toUpperCase() + verification.status.slice(1)} error!`));
    }

    console.log("SMS verify-otp unknown status!", verification);
    return next(new Error("SMS unknown error!"));
  } catch (error) {
    console.error(error);
    throw new Error("internal server error");
  }
});

module.exports = { sendOTPMiddleware, verifyOTPMiddleware };
