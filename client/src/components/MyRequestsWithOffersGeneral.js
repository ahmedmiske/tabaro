...
const MyRequestsWithOffersGeneral = () => {
  ...
  const filtered = useMemo(() => (!urgencyFilter ? requests || [] : (requests || []).filter((r) => (urgencyFilter === 'urgent' ? !!r.isUrgent : !r.isUrgent))), [requests, urgencyFilter]);
    ...
    return (
      <li key={req._id} className={`req-card ${muted ? 'is-muted' : ''}`} onClick={() => openDetails(req._id)}>
        ...
        <div className="rc-meta">
          <span className="badge bg-success">{(req.category || '—')} / {(req.type || '—')}</span>
          ...
          <span className="badge bg-light text-dark border">{req.place || '—'}</span>
        </div>
