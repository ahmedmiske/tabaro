...
const getStatusLabel = (status) => (status === 'fulfilled' ? 'تم الاستلام' : status === 'rated' ? 'تم التقييم' : 'قيد الاستلام');
const getStatusColor = (status) => (status === 'fulfilled' ? 'info' : status === 'rated' ? 'secondary' : 'warning');
...
const isExpired = (deadline, nowMs) => { const d = toDateSafe(deadline); if (!d) return false; return d.getTime() < getNowMs(nowMs); };
...
const MyDonationOffersGeneral = () => {
  ...
  const { activeOffers, inactiveOffers } = useMemo(() => {
    ...
    return { activeOffers: applyStatusFilter(act).sort(byNewest), inactiveOffers: applyStatusFilter(inact).sort(byNewest) };
  ...
  const renderRow = (offer) => {
    ...
    return (
      <tr key={offer._id} onClick={() => openDetails(reqId)} className="clickable-row" style={{ cursor: reqId ? 'pointer' : 'default' }}>
        <td>{ownerName}</td>
        ...
        <td onClick={(e) => e.stopPropagation()}>
          {owner?._id && (
            <Button variant="outline-primary" size="sm" className="me-1 mb-1" onClick={() => navigate(`/chat/${owner._id}`, { state: { from: location.pathname + location.search } })}>
    ...
    return (
      <li key={offer._id} className="card-item" onClick={() => openDetails(reqId)}>
        ...
        <div className="ci-meta">
          ...
          <span className="badge bg-success text-white">{req?.category || '—'}{req?.type ? ` / ${req.type}` : ''}</span>
          <span className={`badge bg-${getStatusColor(offer.status)}`}>{getStatusLabel(offer.status)}</span>
        </div>
  ...
  const section = (title, list, open, setOpen, badgeVariant) => (
    <div className="section-card mb-3">
      <div className="section-head">
        <h6 className="m-0">{title} <Badge bg={badgeVariant} className="ms-1">{list.length}</Badge></h6>
        <Button size="sm" variant="outline-secondary" onClick={() => setOpen((v) => !v)}>{open ? 'إخفاء' : 'عرض'}</Button>
      </div>
      <Collapse in={open}>
        <div>
          {list.length === 0 ? (
  ...
  return (
    <div className="my-donation-offers">
      ...
      {section('العروض غير النشطة', inactiveOffers, openInactive, setOpenInactive, 'secondary')}
