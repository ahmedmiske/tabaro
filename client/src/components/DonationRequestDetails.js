...
import {
  Card, ListGroup, Badge, Button, Spinner, Alert, Form, InputGroup, ProgressBar
...
const formatAmount = (v) =>
  (v === null || v === undefined || v === '' ? '-' : Number(v).toLocaleString('ar-MA'));
...
const makeDocs = (don) => {
  if (Array.isArray(don?.documents) && don.documents.length) {
    return don.documents.map((d) => {
      ...
      const url = absolute
        ? raw
        : `${API_BASE}${raw.startsWith('/') ? raw : `/${raw}`}`.replace(/([^:]\/)\/+/g, '$1');
  ...
  if (Array.isArray(don?.proofDocuments) && don.proofDocuments.length) {
    return don.proofDocuments.map((v) => {
      ...
      const url = absolute
        ? s
        : `${API_BASE}${(isAlreadyUploads ? s : `/uploads/donationRequests/${s}`).replace(
            /\/\/+/g,
            '/'
          )}`.replace(/([^:]\/)\/+/g, '$1');
...
const DonationRequestDetails = () => {
  ...
  useEffect(() => {
    ...
    if (confirmedBefore) {
      ...
      if (localStorage.getItem(LS_BANNER_HIDE) !== '1') {
        setInfoMessage(
          'ℹ️ لقد تم إشعار صاحب الطلب بتبرعكم، ويمكنكم الآن التواصل عبر الوسائل المتاحة.'
        );
      }
    }
    checkExistingOffer();
  ...
  const isOwner =
    Boolean(ownerId && currentUserId && String(ownerId) === String(currentUserId));
  const expired = left !== null && left < 0;
  const recipientId = ownerId || null;

  // ✅ conversationId الحتمي (يربط هذا الطلب بالطرفين)
  ...
  const submitConfirmation = async (e) => {
    ...
    try {
      ...
      if (recipientId && conversationId) {
        ...
        const s = getSocket();
        if (s?.connected) {
          // ✅ انضم للغرفة ثم أرسل الرسالة مع conversationId
          s.emit('joinConversation', { conversationId });
          s.emit('sendMessage', {
            conversationId,
            recipientId,
            content: `💚 ${donorName} أكّد تبرعًا لطلبك — ${req?.category || ''}${
              req?.type ? ` (${req.type})` : ''
            }`,
            requestId: id,
        ...
        <Card.Body>
          <div className="meta-strip d-flex gap-2 flex-wrap mb-3">
            ...
            {req?.deadline && req?.createdAt && (
              <div style={{ minWidth: 140, flex: '1 1 220px' }} className="ms-auto">
          ...
          <ListGroup variant="flush" className="mb-3">
            {!!req.bloodType && (
              <ListGroup.Item>
                <strong>فصيلة الدم:</strong> {req.bloodType}
              </ListGroup.Item>
            )}
            <ListGroup.Item>
            ...
            <ListGroup.Item>
              <strong>المكان:</strong> {req.place || '-'}
            </ListGroup.Item>
            ...
            <ListGroup.Item>
              <strong>آخر مهلة:</strong> {fmtDate(req.deadline)}
            </ListGroup.Item>
            ...
            <ListGroup.Item>
              <strong>تاريخ الإنشاء:</strong> {fmtDate(req.createdAt)}
            </ListGroup.Item>
                  ...
                  {docs.map((d, idx) => (
                    <div className="doc-tile" key={idx}>
                            ...
                            onError={(e) => {
                              e.currentTarget.replaceWith(
                                Object.assign(document.createElement('i'), {
                                  className: 'far fa-file generic-icon',
                                })
                              );
                            }}
                          />
                      ...
                      <div className="doc-actions">
                        ...
                        <a className="btn btn-sm btn-outline-secondary" href={d.url} download>
                          تنزيل
                        </a>
                      </div>
                    </div>
                ...
                <Form.Group className="mb-3">
                  ...
                  {evidenceFiles?.length > 0 && (
                    <div className="text-muted mt-1">
                      تم اختيار {evidenceFiles.length} ملف/ملفات.
                    </div>
                  )}
                </Form.Group>
                <Alert variant="light" className="border">
                  بعد إرسال التأكيد، سيُخطر صاحب الطلب فورًا وتظهر وسائل التواصل مباشرة. نوصي بإرفاق
                  إثبات إن وُجد.
                </Alert>
          ...
          {activeSection === 'chat' && !isOwner && recipientId && conversationId && (
            <div className="action-panel">
              <h6 className="fw-bold mb-1">محادثة مع {ownerName}</h6>
              <div className="chat-topic">
                موضوع المحادثة: {req.category || '—'} — {req.type || '—'}
                {req.place ? ` • ${req.place}` : ''}
              </div>
              {/* ✅ تمرير conversationId + recipientId */}
